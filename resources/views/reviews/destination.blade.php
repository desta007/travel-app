...
@section('title', 'Reviews for ' . $destination->name . ' - TravelApp')
@section('description', 'Read reviews and ratings for ' . $destination->name)
        ...
        <div class="col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('destinations.show', $destination) }}">{{ $destination->name }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Reviews</li>
                </ol>
            </nav>
            <h1 class="fw-bold mb-3">Reviews for {{ $destination->name }}</h1>
        </div>
    </div>
    ...
    <div class="row">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-8">
                            @for($rating = 5; $rating >= 1; $rating--)
                                @php
                                    $count = $destination->reviews()->where('rating', $rating)->count();
            ...
            <div class="mb-4">
                        ...
                        <div class="card-body">
                            ...
                            @if($review->travel_date)
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <small class="text-muted">Traveled {{ $review->travel_date->format('F Y') }}</small>
                ...
                @else
                    <div class="text-center py-5">
                        ...
                        @auth
                            <a href="{{ route('reviews.create', ['type' => 'destination', 'id' => $destination->id]) }}" class="btn btn-primary">
        ...
        <div class="col-lg-4">
            <div class="sticky-top" style="top: 2rem;">
                <!-- Write Review -->
                @auth
                    @if($destination->reviews()->where('user_id', auth()->id())->count() == 0)
                    <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <h5 class="fw-bold mb-3">About {{ $destination->name }}</h5>
                        <img src="{{ $destination->image_url ?: 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80' }}" 
                             alt="{{ $destination->name }}" class="img-fluid rounded mb-3">
                        <p class="text-muted">{{ Str::limit($destination->description, 150) }}</p>

...
@section('title', 'Reviews for ' . $activity->name . ' - TravelApp')
@section('description', 'Read reviews and ratings for ' . $activity->name)
        ...
        <div class="col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('activities.show', $activity) }}">{{ $activity->name }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Reviews</li>
                </ol>
            </nav>
            <h1 class="fw-bold mb-3">Reviews for {{ $activity->name }}</h1>
        </div>
    </div>
    ...
    <div class="row">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-8">
                            @for($rating = 5; $rating >= 1; $rating--)
                                @php
                                    $count = $activity->reviews()->where('rating', $rating)->count();
            ...
            <div class="mb-4">
                        ...
                        <div class="card-body">
                            ...
                            @if($review->travel_date)
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <small class="text-muted">Experienced {{ $review->travel_date->format('F Y') }}</small>
                ...
                @else
                    <div class="text-center py-5">
                        ...
                        @auth
                            <a href="{{ route('reviews.create', ['type' => 'activity', 'id' => $activity->id]) }}" class="btn btn-primary">
        ...
        <div class="col-lg-4">
            <div class="sticky-top" style="top: 2rem;">
                <!-- Write Review -->
                @auth
                    @if($activity->reviews()->where('user_id', auth()->id())->count() == 0)
                    <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <h5 class="fw-bold mb-3">About {{ $activity->name }}</h5>
                        <img src="{{ $activity->image_url ?: 'https://images.unsplash.com/photo-1551632811-561732d1e306?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80' }}" 
                             alt="{{ $activity->name }}" class="img-fluid rounded mb-3">
                        <p class="text-muted">{{ Str::limit($activity->description, 150) }}</p>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">
                                <i class="fas fa-map-marker-alt me-1"></i>{{ $activity->destination->name }}
                            </small>
